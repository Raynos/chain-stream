...
function reduce(iterator, combination, initial, value, write, end) {
    if (arguments.length === 5) {
        end = write
        write = value
        value = initial
        initial = combination
        combination = returnLast
    }
    ...
    iterator(accumulator, value, function (err, result) {
        ...
        var accumulator = reductionsMap.get(self)

        accumulator = combination(accumulator, result)

        reductionsMap.set(self, accumulator)

        end(null, accumulator)
    })
}

function returnLast(accumulat, result) {
    return result
}
