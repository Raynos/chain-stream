...
var forEachObject = require("for-each")
    ...
    , methods = {
        concat: concat
        , first: first
        , flatten: Async(transform, flatten)
        , flattenSerial: Async(transform, flatten, true)
        , last: last
        , lazyPipe: lazyPipe
        , transform: transform
    }
...
forEachObject(transformations, function (func, name) {
    var asyncFunc = Async(transform, func)
        , syncFunc = Sync(asyncFunc)
        , asyncSerialFunc = Async(transform, func, true)

    methods[name + "Serial"] = asyncSerialFunc
...
forEachObject(complexTransformations, function (data, name) {
    var transform = data.transform
        ...
        , syncFunc = func(methods[transform])
        , asyncSerialFunc = func(methods[transform + "Serial"])
